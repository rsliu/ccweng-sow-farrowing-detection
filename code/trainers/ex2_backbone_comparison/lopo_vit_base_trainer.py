# Organized filename: lopo_vit_base_trainer.py
# Purpose: LOPO trainer for ViT-B in the backbone comparison.
# Original source: train_vit.py

# -*- coding: utf-8 -*-
"""
purevit_lopo_train_v2.py

Leak-safe LOPO training for pure ViT using the standardized MSFUNet evaluation protocol:
...
- WeightedRandomSampler (alpha,beta,gamma); if alpha=beta=gamma=0 -> shuffle (no replacement)
...
Run:
  python purevit_lopo_train_v2.py --path_d Dataset/full --roi_cfg roi.json --img_size 224 -e 55 -b 16 --nw 4

Notes:
- Binary classification assumed for threshold sweep (2 classes). For >2 classes, threshold sweep is skipped.
"""
...
def build_transforms(aug_preset: str, img_size: int, use_color_align_eval: bool=False):
    ...
    elif aug_preset in ("G", "pig_gray", "pig_gray"):
        tf_train = transforms.Compose([
            transforms.Resize((base_resize, base_resize)),
            transforms.RandomResizedCrop(img_size, scale=(0.6, 1.0), ratio=(0.8, 1.25)),
            transforms.RandomHorizontalFlip(0.5),
...
# =========================
# ViT (multi-layer hidden concat head)
# =========================
class CustomViTForImageClassification(ViTForImageClassification):
    ...
    def forward(self, pixel_values=None, **kwargs):
        out = self.vit(pixel_values, output_hidden_states=True)
        hs = out.hidden_states  # list[(B,N,D)], embeddings at idx=0
        picked = [hs[i] for i in self.layer_indices]
        pooled = [t.mean(dim=1) for t in picked]  # mean over tokens
        feat = torch.cat(pooled, dim=1)
        logits = self.classifier(feat)
        return SequenceClassifierOutput(logits=logits)

def setup_model(num_classes: int, freeze_option=0, total_layers=12, layer_pick=None, img_size=224):
    layer_indices = sorted(set(layer_pick or {
        total_layers - 1,
        total_layers // 5,
        2 * total_layers // 5,
        3 * total_layers // 5,
    ...
    backbone_id = "google/vit-base-patch16-384" if img_size >= 320 else "google/vit-base-patch16-224-in21k"
    cfg = AutoConfig.from_pretrained(backbone_id, num_labels=num_classes)
    cfg.hidden_dropout_prob = 0.2
    cfg.attention_probs_dropout_prob = 0.2

    model = CustomViTForImageClassification.from_pretrained(
        backbone_id, config=cfg, layer_indices=layer_indices, ignore_mismatched_sizes=True
    )

    in_dim = len(layer_indices) * model.config.hidden_size
    model.classifier = nn.Sequential(
        nn.Linear(in_dim, 3072), nn.ReLU(), nn.Dropout(0.5),
...
def build_param_groups(model, backbone_lr, head_lr, weight_decay, freeze_first_k=0):
    # freeze first K blocks if requested
    if freeze_first_k > 0 and hasattr(model.vit, "encoder") and hasattr(model.vit.encoder, "layer"):
        enc = model.vit.encoder.layer
        for blk in enc[:min(freeze_first_k, len(enc))]:
    ...
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue
        (back_params if n.startswith("vit.") else head_params).append(p)
...
def _gather_logits_targets(model, loader, device, tta_hflip=False):
    ...
    for x, y in loader:
        ...
        z = net(x).logits
        if tta_hflip:
            z = 0.5 * (z + net(torch.flip(x, dims=[-1])).logits)
...
def eval_collect(model, loader, device, temp_model: Optional[_Temp]=None, tta_hflip=False):
    ...
    for x, y in loader:
        ...
        with autocast(device_type="cuda", enabled=use_amp):
            logits = net(x).logits
            if tta_hflip:
                logits = 0.5 * (logits + net(torch.flip(x, dims=[-1])).logits)
...
def threshold_sweep_binary(y_true: np.ndarray, prob1: np.ndarray, thr_min=0.25, thr_max=0.75, steps=41):
    ...
    for t in np.linspace(thr_min, thr_max, steps):
        ...
        # macro-acc: (acc_pos + acc_neg)/2 == (rec + spec)/2
        macro = 0.5 * (rec + spec)
        if macro > best["macro"]:
...
def adapt_bn(model, loader, device, max_batches=50):
    """
    Update BN running stats using (train) data.
    Note: ViT usually has no BN, but keep for consistency (harmless).
    """
    model.train()
    net = model.module if hasattr(model, "module") else model
...
def run_lopo(args):
    ...
    for fold_idx, test_pig in enumerate(pigs_to_run, start=1):
        ...
        # model
        model, used_layers = setup_model(num_classes, freeze_option=args.f, img_size=args.img_size)
        ...
        # EMA  (IMPORTANT: later we only save/load ema.module.state_dict())
        ema = None
        ...
        with open(log_path_txt, "w", encoding="utf-8") as ftxt:
            ...
            ftxt.write(f"Used layers: {used_layers}\n")
        ...
        for ep in range(1, args.e + 1):
            ...
            # eval backbone: use EMA weights if enabled, else raw model
            eval_model = ema if ema is not None else (model.module if isinstance(model, nn.DataParallel) else model)

            # temp scaling for val_tune (optional)
            temp_for_eval = None
            ...
            # best temp (savebest): fit at new best
            ...
            # ===== SAVE BEST (IMPORTANT FIX) =====
            if va_loss < best_val:
                best_val = va_loss
                best_epoch = ep
                if ema is not None:
                    # Save EMA weights (module weights) so load_state_dict works on base model later
            ...
            if (ep % args.val_full_every == 0) or (ep == args.e) or should_stop:
                ...
                # threshold sweep on val_full (binary only)
                if num_classes == 2:
                    ...
                    # carry-to-test: keep best macro threshold seen so far
        ...
        # Evaluate val_full and test using carried threshold (binary)
        y_true_te, _, prob1_te = eval_collect(model, test_loader, device, temp_model=best_temp_for_test, tta_hflip=args.tta_hflip)
        y_true_vf, _, prob1_vf = eval_collect(model, val_full_loader, device, temp_model=best_temp_for_test, tta_hflip=args.tta_hflip)

        # default predictions
        if num_classes == 2:
            y_pred_te = (prob1_te >= best_thr).astype(np.int64)
            y_pred_vf = (prob1_vf >= best_thr).astype(np.int64)
            cm_te = confusion_counts(y_true_te, y_pred_te, 2)
            cm_vf = confusion_counts(y_true_vf, y_pred_vf, 2)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser("LOPO — pure ViT + ROI + Letterbox + robust aug + EarlyStop + EMA + Temp + thr-sweep + TTA")

    # data & output
    ap.add_argument("--path_d", type=str, required=True)
    ap.add_argument("--path_r", type=str, default="./Result/lopo_vit")
    ap.add_argument("--path_m", type=str, default="./Model/lopo_vit")
    ...
    ap.add_argument("--freeze_first_k", type=int, default=0)
